...
const Knowledge = () => {
  ...
  // Get articles and FAQs from dynamic content (respecting language)
  const articles: any[] = !isEn && knowledge.marathi?.articles?.length > 0
    ? knowledge.marathi.articles
    : (isEn && knowledge.articles?.length > 0 
        ? knowledge.articles 
        : (t('knowledge.articles', { returnObjects: true }) as any[]));
    
  const faqs: any[] = !isEn && knowledge.marathi?.faqs?.length > 0
    ? knowledge.marathi.faqs
    : (isEn && knowledge.faqs?.length > 0
        ? knowledge.faqs
        : (t('knowledge.faqs', { returnObjects: true }) as any[]));

  const categoryMap: Record<string, string> = {
    'All': t('knowledge.all'),
    'Criminal Law': t('knowledge.criminalLaw'),
    'Civil Litigation': t('knowledge.civilLaw'),
    'Family Law': t('knowledge.familyLaw'),
    'Property Law': t('knowledge.propertyLaw'),
    'Corporate Law': t('knowledge.corporateLaw'),
    'Consumer Law': t('knowledge.consumerLaw'),
  };
  ...
  return (
    <Layout>
      ...
      <section className="section-padding bg-background">
        <div className="container-legal">
          ...
          {filteredArticles.filter(a => a.featured).length > 0 && (
            <div className="mb-12">
              <h2 className="heading-section text-foreground mb-6">{t('knowledge.featuredArticles')}</h2>
                  ...
                  <article key={article.id} className="card-premium p-6 group cursor-pointer" onClick={() => setSelectedArticle(article)}>
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-border">
                      ...
                      <span className="flex items-center gap-1 text-accent text-sm font-medium group-hover:gap-2 transition-all">
                        {t('knowledge.readMore')} <ArrowRight className="w-4 h-4" />
                      </span>
                    </div>
                  </article>
                ))}
        ...
        <div className="container-legal">
          <div className="max-w-3xl mx-auto">
            <div className="text-center mb-12">
              <span className="text-accent text-sm font-semibold tracking-widest uppercase mb-4 block">{t('knowledge.commonQuestions')}</span>
              <h2 className="heading-section text-foreground mb-4">{t('knowledge.frequentlyAskedQuestions')}</h2>
      ...
      <section className="section-padding bg-navy">
        <div className="container-legal text-center">
          <h2 className="heading-section text-cream mb-4">{t('knowledge.ctaTitle')}</h2>
          <p className="text-cream/70 mb-8 max-w-xl mx-auto">
            {t('knowledge.ctaSubtitle')}
          </p>
          <Link to="/inquiry">
            <Button className="btn-gold px-8 py-6 h-auto text-lg">
              {t('knowledge.bookConsultation')}
